pipeline {
  ...
  environment {
    ...
    SONARQUBE_TOKEN = 'sqp_0f775cd820a27679e34a34aeb46c1569602ca981' // Define este secreto en Jenkins
  }
  ...
  stages {
    ...
    stage('Análisis SonarQube') {
      ...
      steps {
        withSonarQubeEnv('SonarQube') {
          sh """
            mvn sonar:sonar \
              -Dsonar.projectKey=CALCULADORA_API \
    ...
    stage('Instalar Chrome y ChromeDriver') {
      steps {
        sh """
          su apt-get update 
          su apt-get install chromium-chromedriver chromium
          su ln -s /usr/lib/chromium-browser/chromedriver /usr/local/bin/chromedriver
